...
public class ImpreseManager {
    ...
    Logger logger = Logger.getLogger(ImpreseManager.class);

    private ImpreseDao impreseGaraDao;

    /**
     * @param impreseGaraDao
     *            the impreseGaraDao to set
     */
    public void setImpreseGaraDao(ImpreseDao impreseGaraDao) {
        this.impreseGaraDao = impreseGaraDao;
    }
	...
	public DatiImpresaDocument getDatiImpresa(String token, Date dataPartenzaCessati) {
		...
		// richiesti
		DatiImpresaDocument documento = DatiImpresaDocument.Factory
				.newInstance();
	...
	private DatoAnnuoImpresaAggiornabileType getDatiAnnui(String codiceImpresa,
		...
		if (risultato == null) {
			risultato = DatoAnnuoImpresaAggiornabileType.Factory.newInstance();
