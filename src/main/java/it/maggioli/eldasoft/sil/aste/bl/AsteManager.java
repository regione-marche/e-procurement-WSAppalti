...
import it.maggioli.eldasoft.sil.aste.ws.WSAsteSoapImpl;
...
public class AsteManager {
	 ...
	 */	 
	public static final boolean CLASSIFICA_AL_RIBASSO = true;
	
	Logger logger = Logger.getLogger(AsteManager.class);
    ...
    /**
     * @param asteDao
     *            asteDao da settare internamente alla classe.
     */
    public void setAsteDao(AsteDao asteDao) {
    	this.asteDao = asteDao;
    }    
    ...
    /**
	 * @param datiCodificatiManager the datiCodificatiManager to set
	 */
	 ...
	 */
    private boolean isRilancioMigliore(Double rilancio1, Double rilancio2) {
    	if(rilancio1 == null || rilancio2 == null) {
    		return false;
    	}
    	if(CLASSIFICA_AL_RIBASSO) {
	...
	public Long insertRilancioAsta(
		...
		try {
			...
			// verifica se l'asta è attiva ed il rilancio è nei termini dell'asta...
			...
			if(continua) {
				...
				Long newId = new Long( this.genChiaviManager.getNextId("AERILANCI") );
				...
				double totalePrezziUnitari = 0.0;
				if(prezziUnitari != null && prezziUnitari.size() > 0) {					
					// calcola il totale prezzi unitari...
					double segno = Math.signum(offerta);
					List<VoceDettaglioAstaType> tmp = new ArrayList<VoceDettaglioAstaType>();
					for(int i = 0; i < prezziUnitari.size(); i++) {
						if(prezziUnitari.get(i).isSoloSicurezza() ||
						   prezziUnitari.get(i).isNonSoggettoRibasso()) {
							// ignora le voci "sicurezza" e "non soggette"
						} else {
							totalePrezziUnitari += (prezziUnitari.get(i).getQuantita().doubleValue() *
									   				prezziUnitari.get(i).getAstePrezzoUnitario().doubleValue());
							tmp.add(prezziUnitari.get(i));
						}
					}						
					prezziUnitari = tmp;	
					
					// recupero il totale non soggetto a ribasso della gara... 
					double totaleNonSoggetto = this.asteDao.getTotaleNonSoggettoPrezziUnitari(codiceGara);					
					
					if(asta.getTipoOfferta() == WSAsteSoapImpl.TIPO_OFFERTA_IMPORTI) {
						offerta = segno * 
								 (totalePrezziUnitari + totaleNonSoggetto);	
					}
				}
				...
				for(int i = 0; i < classifica.size(); i++) {									
					...
					if(isRilancioMigliore(offerta, classifica.get(i).getImporto())) {
				...
				// è sempre possibile rilanciare, anche sulla propria posizione!
				// Il rilancio viene inserito se è migliore del precedente...
				...
				if(newPosizione > 0) {
					...
					if(asta.getTipoOfferta() == WSAsteSoapImpl.TIPO_OFFERTA_RIBASSI
					   && (prezziUnitari != null && prezziUnitari.size() > 0)) {
						rilancio.setImporto(totalePrezziUnitari);
					}						
					...
					if(prezziUnitari != null && prezziUnitari.size() > 0 && newId > 0) {
						Long asteId = new Long( genChiaviManager.getNextId("AERILPRE", prezziUnitari.size()) );
						for(int i = 0; i < prezziUnitari.size(); i++) {
							prezziUnitari.get(i).setAsteId(asteId++);
						}
						this.asteDao.insertPrezziUnitari(newId, prezziUnitari);
					}					
				} else {
					// nessun nuovo rilancio inserito
					// (Es: un utente rilancia quando è già il miglior offerente)
					throw new NessunRilancioInseritoException("Nessun nuovo rilancio inserito.");
				}
										
				// il rilancio è andato a buon fine...
