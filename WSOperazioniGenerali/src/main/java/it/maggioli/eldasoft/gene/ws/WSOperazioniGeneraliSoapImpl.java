...
public class WSOperazioniGeneraliSoapImpl implements WSOperazioniGeneraliSoap {
	...
	private static final Logger LOG = LoggerFactory.getLogger(WSOperazioniGeneraliSoapImpl.class);
...
//	}
	...
	public static String buildXML(List<DatoCodificatoType> listaDati) {
		//Ricopiato da ListaDatiCodificatiDocument perchè la nuova versione contiene un oggetto factory che non perette
		//Di richiamare la classe Factory
		ListaDatiCodificatiDocument documento =
				(ListaDatiCodificatiDocument)
						XmlBeans.getContextTypeLoader().newInstance(ListaDatiCodificatiDocument.type, null);

		ListaDatiCodificatiType lista = documento.addNewListaDatiCodificati();
		DatoCodificatoType[] datoCodificatoTypes = listaDati.stream().filter(Objects::nonNull).toArray(DatoCodificatoType[]::new);
		lista.setElementoArray(datoCodificatoTypes);
	...
	@Override
	public String getFormeGiuridicheConSocietaCooperativa() {
		LOG.debug("getElencoTipiNaturaGiuridica: inizio metodo");

		List<DatoCodificatoType> listaTipi = datiCodificatiManager.getFormeGiuridicheCooperative()
				.stream()
				.filter(dati -> StringUtils.isNotEmpty(dati.getDescrizione()))
	...
	@Override
	public String getSocietaCooperativa() {
		LOG.debug("getElencoTipiNaturaGiuridica: inizio metodo");

		List<DatoCodificatoType> listaTipi = datiCodificatiManager.getTipiTab1("G_074");
		String documentoXML = WSOperazioniGeneraliSoapImpl.buildXML(listaTipi);
	...
	@Override
	public ErroreOutType deleteAllegatoProtocollo(java.lang.String entita, java.lang.String applicativo, long id) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("deleteAllegatoProtocollo(" + entita + "," + applicativo + "," + id
					+ "): inizio metodo");
		}

		ErroreOutType risultato = new ErroreOutType();
		try {
			this.comunicazioniManager.deleteAllegatoProtocollo(entita, applicativo, id);
		} catch (DataAccessException e) {
			LOG.error(
					"Errore inaspettato durante l'interazione con la base dati",
					e);
		...
		LOG.debug("deleteAllegatoProtocollo: fine metodo");
		return risultato;
    }

	@Override
	public String getNazioniCodificateDGUE(String nazione) {
		return datiCodificatiManager.getNazioniCodificateDGUE(nazione);
	}
